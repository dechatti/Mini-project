...
def concordance(document):  # Turns a sentence into a word frequency dictionary
...
class VectorCompare:
    def magnitude(self, concordance):  # Calculates the length of the vector
...
'''index = { #Refactoring was done here
...
search_vector = v.concordance(searchterm.lower())
matches = []

for i in index:

    relation = v.relation(search_vector, index[i])
    if relation != 0:
